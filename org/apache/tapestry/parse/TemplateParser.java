...
public class TemplateParser {
	...
	protected static class TemplateTokenFactory {
		...
		public OpenToken createOpenToken(String tagName, String jwcId,
				String type, ILocation location) {
		...
		public TextToken createTextToken(char[] templateData, int blockStart,
				int end, ILocation templateLocation) {
			return new TextToken(templateData, blockStart, end,
					templateLocation);
		}
	...
	public TemplateToken[] parse(char[] templateData,
			ITemplateParserDelegate delegate, IResourceLocation resourceLocation)
		...
		try {
			...
			result = (TemplateToken[]) _tokens
					.toArray(new TemplateToken[_tokens.size()]);
		} finally {
			afterParse();
		}
	...
	protected void templateParseProblem(String message, ILocation location,
			int line, int cursor) throws TemplateParseException {
	...
	protected void templateParseProblem(ApplicationRuntimeException exception,
			int line, int cursor) throws ApplicationRuntimeException {
		throw exception;
	}
	...
	private void skipComment() throws TemplateParseException {
		...
		while (true) {
			if (_cursor >= length)
				templateParseProblem(Tapestry.format(
						"TemplateParser.comment-not-ended", Integer
								.toString(startLine)), new Location(
	...
	private void addTextToken(int end) {
		...
		if (_blockStart <= end) {
			TemplateToken token = _factory.createTextToken(_templateData,
					_blockStart, end, _templateLocation);
	...
	private void startTag() throws TemplateParseException {
		...
		while (_cursor < length) {
			...
			if (ch == '/' || ch == '>' || Character.isWhitespace(ch)) {
				tagName = new String(_templateData, cursorStart + 1, _cursor
						- cursorStart - 1);
		...
		while (!endOfTag) {
			if (_cursor >= length) {
				String key = (tagName == null) ? "TemplateParser.unclosed-unknown-tag"
						: "TemplateParser.unclosed-tag";

				templateParseProblem(Tapestry.format(key, tagName, Integer
						.toString(startLine)), startLocation, startLine,
						cursorStart);
			...
			case COLLECT_ATTRIBUTE_NAME:
				...
				if (ch == '=' || ch == '/' || ch == '>'
					...
					attributeName = new String(_templateData,
							attributeNameStart, _cursor - attributeNameStart);
			...
			case WAIT_FOR_ATTRIBUTE_VALUE:
				...
				if (ch == '/' || ch == '>')
					templateParseProblem(Tapestry.format(
							"TemplateParser.missing-attribute-value", tagName,
							Integer.toString(_line), attributeName),
				...
				if (ch == '\'' || ch == '"') {
					...
					attributeBeginEvent(attributeName, _line,
							attributeValueStart);
					break;
				}
			...
			case COLLECT_QUOTED_VALUE:
				...
				if (ch == quoteChar) {
					String attributeValue = new String(_templateData,
							attributeValueStart, _cursor - attributeValueStart);
		...
		String localizationKey = findValueCaselessly(
				LOCALIZATION_KEY_ATTRIBUTE_NAME, _attributes);
		...
		if (localizationKey != null && tagName.equalsIgnoreCase("span")
			...
			if (_ignoring)
				templateParseProblem(Tapestry.format(
						"TemplateParser.component-may-not-be-ignored", tagName,
						Integer.toString(startLine)), startLocation, startLine,
						cursorStart);
			...
			TemplateToken token = _factory.createLocalizationToken(tagName,
					localizationKey, raw, attributes, startLocation);
	 ...
	 * author glongman@intelligentworks.com
	 */
	protected void attributeBeginEvent(String attributeName, int startLine,
			int cursorPosition) {
	}
	...
	private void processComponentStart(String tagName, String jwcId,
			boolean emptyTag, int startLine, int cursorStart,
			ILocation startLocation) throws TemplateParseException {
		...
		if (_patternMatcher.matches(jwcId, _implicitIdPattern)) {
			...
			String libraryId = match
					.group(IMPLICIT_ID_PATTERN_LIBRARY_ID_GROUP);
			String simpleType = match
					.group(IMPLICIT_ID_PATTERN_SIMPLE_TYPE_GROUP);
			...
			if (!isRemoveId) {
				if (!_patternMatcher.matches(jwcId, _simpleIdPattern))
					templateParseProblem(Tapestry.format(
							"TemplateParser.component-id-invalid", tagName,
							Integer.toString(startLine), jwcId), startLocation,
							startLine, cursorStart);

				if (!_delegate.getKnownComponent(jwcId))
					templateParseProblem(Tapestry.format(
							"TemplateParser.unknown-component-id", tagName,
							Integer.toString(startLine), jwcId), startLocation,
							startLine, cursorStart);
		...
		if (_ignoring && ignoreBody)
			templateParseProblem(Tapestry.format(
					"TemplateParser.nested-ignore", tagName, Integer
							.toString(startLine)), new Location(
	...
	private void pushNewTag(String tagName, int startLine, boolean isRemoveId,
			boolean ignoreBody) {
		Tag tag = new Tag(tagName, startLine);
	...
	private void processContentTag(String tagName, int startLine,
			int cursorStart, boolean emptyTag) throws TemplateParseException {
		if (_ignoring)
			templateParseProblem(Tapestry.format(
					"TemplateParser.content-block-may-not-be-ignored", tagName,
					Integer.toString(startLine)), new Location(
					_resourceLocation, startLine), startLine, cursorStart);

		if (emptyTag)
			templateParseProblem(Tapestry.format(
					"TemplateParser.content-block-may-not-be-empty", tagName,
					Integer.toString(startLine)), new Location(
					_resourceLocation, startLine), startLine, cursorStart);
	...
	private void addOpenToken(String tagName, String jwcId, String type,
			ILocation location) {
		OpenToken token = _factory.createOpenToken(tagName, jwcId, type,
				location);
		_tokens.add(token);
	...
	private void addAttributeToToken(OpenToken token, String name,
		...
		if (pos > 0) {
			...
			if (prefix.equals(OGNL_EXPRESSION_PREFIX)) {
				token.addAttribute(name, AttributeType.OGNL_EXPRESSION,
						extractExpression(attributeValue.substring(pos + 1)));
				return;
			}

			if (prefix.equals(LOCALIZATION_KEY_PREFIX)) {
				token.addAttribute(name, AttributeType.LOCALIZATION_KEY,
	...
	private void closeTag() throws TemplateParseException {
		...
		while (true) {
			if (_cursor >= length)
				templateParseProblem(Tapestry.format(
						"TemplateParser.incomplete-close-tag", Integer
								.toString(startLine)), startLocation,
						startLine, cursorStart);
		...
		while (stackPos >= 0) {
			...
			if (tag._mustBalance)
				templateParseProblem(Tapestry.format(
						...
						new Object[] { tagName, Integer.toString(startLine),
								tag._tagName, Integer.toString(tag._line) }),
						startLocation, startLine, cursorStart);
		...
		if (stackPos < 0) {
			if (_delegate.getComponent().getSpecification()
					.isPageSpecification()) {

				templateParseProblem(Tapestry.format(
						"TemplateParser.unmatched-close-tag", tagName, Integer
								.toString(startLine)), startLocation,
						startLine, cursorStart);
			}
		}
		...
		if(tag != null){
			...
			if (tag._component) {
				...
				_tokens.add(_factory
						.createCloseToken(tagName, getCurrentLocation()));
			} else {
			...
			//test begin add && i >= 0
			for (int i = _stack.size() - 1; i >= stackPos && i >= 0; i--)
				_stack.remove(i);
			//test end
	...
	private String extractExpression(String input) {
		...
		outer: while (cursor < inputLength) {
			for (int i = 0; i < CONVERSIONS.length; i += 2) {
				...
				if (input.substring(cursor, cursor + entityLength).equals(
						entity)) {
					buffer.append(value);
					cursor += entityLength;
					continue outer;
